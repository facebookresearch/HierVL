...
from trainer import Multi_Trainer_dist_HT100M_Classification
...
def main_worker(gpu, ngpus_per_node, args, config): #TODO: Take config as input
    ...
    #trainable_params = [p for (name, p) in model.named_parameters() if p.requires_grad and 'ht100m' not in name]
    #ht100m_parameters = [param for (name, param) in model.named_parameters() if 'ht100m' in name]
    #optimizer = config.initialize('optimizer', transformers, [{'params': trainable_params}, {'params': ht100m_parameters, 'lr': 3e-5}])
    ...
    trainer = Multi_Trainer_dist_HT100M_Classification(args, model, loss, metrics, optimizer,
                      config=config,
